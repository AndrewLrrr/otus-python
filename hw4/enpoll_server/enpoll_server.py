...
class SimpleHTTPRequestHandler(object):
    ...
    def do_GET(self):
        ...
        if status == OK:
            return self.response(self.set_body())
        return self.response(status)

    def do_HEAD(self):
        return self.response(self.set_head())

    def run(self):
        return self.handle_request()

    def handle_request(self):
        response_code = self.parse_request()
        if response_code != OK:
            return self.response(response_code)
        method = getattr(self, 'do_' + self.method)
        return method()

    def parse_request(self):
        request_lines = self.request_raw.split('\r\n')
    ...
    def response(self, code):
        ...
        return '%s\r\n%s\r\n\r\n%s' % (first_line, headers, (self.body if code == OK else ''))
            ...
            while True:
                ...
                for fileno, event in events:
                    if fileno == self.sock.fileno():
                        ...
                        logging.debug(
                            'Connected | P: %s | PID: %d', multiprocessing.current_process().name, os.getpid())
                        ...
                        if EOL1 in requests[fileno] or EOL2 in requests[fileno]:
                            ...
                            response = self.request_handler(requests[fileno]).run()
